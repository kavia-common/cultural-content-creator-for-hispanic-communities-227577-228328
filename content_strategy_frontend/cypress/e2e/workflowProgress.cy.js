describe("Workflow Progress (outline)", () => {
  it("supports pause/resume, request changes, approve, and advancing steps", () => {
    ...
    cy.get('[data-testid="workflow-step-Strategist"]').should(
      "have.attr",
      "aria-current",
      "step",
    );
    ...
    cy.get('[data-testid="workflow-step-Strategist"]').contains(
      /changes requested|cambios solicitados/i,
    );
    ...
    cy.get('[data-testid="workflow-advance"]')
      .should("not.be.disabled")
      .click();

    // Next step becomes current
    cy.get('[data-testid="workflow-step-Copywriter"]').should(
      "have.attr",
      "aria-current",
      "step",
    );
