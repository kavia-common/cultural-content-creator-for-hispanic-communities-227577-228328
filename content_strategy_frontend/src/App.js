...
import {
  WorkflowProvider,
  getWorkflowRoleUiStateFromSteps,
  useWorkflow,
} from "./state/workflow";
...
function MainApp() {
  ...
  const artifactId = useMemo(
    () => (topic || "default").trim().toLowerCase() || "default",
    [topic],
  );
  ...
  const handleGenerateCaptions = async ({
    ...
    pushMessage({
      kind: "success",
      messageKey: "captions.messages.generated",
      live: "polite",
    });
  };
    ...
    <>
      ...
      >
        <AppLayout.LeftPanel ariaLabel={t("panels.workflow")}>
          ...
          >
            ...
            <HelpTooltip label={t("help.inline.openLabel")}>
              {t("help.inline.workflow")}
            </HelpTooltip>
          </div>
        ...
        <AppLayout.CenterPanel ariaLabel={t("panels.create")}>
          ...
          <TopicInput
            ...
            onConfirmed={(confirmedTopic) => {
              ...
              pushMessage({
                kind: "success",
                messageKey: "messages.topicConfirmed",
              });
            }}
          />
          ...
          <CaptionsPanel
            ...
            onEditCaption={(id, nextText) => {
              setCaptions((prev) =>
                prev.map((c) => (c.id === id ? { ...c, text: nextText } : c)),
              );
              pushMessage({
                kind: "success",
                messageKey: "captions.messages.saved",
                live: "polite",
              });
            }}
            onApproveCaption={(id) => {
              ...
              if (chosen) {
                setPreviewContent({
                  title: t("preview.placeholderTitle", {
                    topic: topic || t("captions.previewFallbackTopic"),
                  }),
                  body: chosen.text,
                  channel: "facebook",
                });
              ...
              pushMessage({
                kind: "success",
                messageKey: "captions.messages.approved",
                live: "polite",
              });
            }}
          />
      ...
      <OnboardingTour
        ...
        onFinish={() => {
          ...
          pushMessage({
            kind: "success",
            messageKey: "messages.onboardingComplete",
          });
        }}
      />
