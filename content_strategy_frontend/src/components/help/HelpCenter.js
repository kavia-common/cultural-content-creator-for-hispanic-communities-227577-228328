...
const SECTIONS = ['gettingStarted', 'topicInput', 'workflow', 'previewExport', 'accessibility'];
...
export default function HelpCenter({ isOpen, onClose, onOpenOnboarding }) {
  ...
  const sectionTitle = useMemo(() => t(`helpCenter.sections.${active}.title`), [t, active]);
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={title}>
      <div style={{ display: 'grid', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <button type="button" className="btn btnSecondary" onClick={onOpenOnboarding}>
