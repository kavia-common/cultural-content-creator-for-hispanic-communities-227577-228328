...
function variationLabelKey(type) {
  ...
  if (type === CaptionVariationType.question)
    return "captions.variation.question";
...
export default function CaptionsPanel({
  ...
  const orderedCaptions = useMemo(() => {
    ...
    const order = [
      CaptionVariationType.long,
      CaptionVariationType.short,
      CaptionVariationType.question,
    ];
    ...
    <section className="card" aria-label={title}>
      ...
      <div className="cardHeader">
        ...
        {status === "loading" ? (
          ...
          <span
            className="badge badgeComplete"
            aria-label={t("captions.status.success")}
          >
            <span className="badgeDot" aria-hidden="true" />
      ...
      {status === "error" ? (
        <div
          className="callout calloutError"
          role="alert"
          style={{ marginTop: 8 }}
        >
