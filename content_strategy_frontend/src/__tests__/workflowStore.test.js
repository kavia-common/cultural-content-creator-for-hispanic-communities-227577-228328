...
test("Pause blocks advancing even after approval until resumed (contract)", () => {
  ...
  state = setStep(state, state.currentStepId, {
  ...
  const canAdvance =
    state.steps.find((s) => s.id === state.currentStepId)?.status ===
    WorkflowStepStatus.approved;
  expect(canAdvance).toBe(false);

  // Resume: should no longer be paused
  state = setStep(state, state.currentStepId, {
    status: WorkflowStepStatus.awaiting_review,
    pauseReason: "",
  });
  expect(state.steps.find((s) => s.id === state.currentStepId)?.status).toBe(
...
test("Request changes updates status and stores comment", () => {
  ...
  state = setStep(state, stepId, {
    status: WorkflowStepStatus.changes_requested,
    changesComment: "Fix tone",
  });
